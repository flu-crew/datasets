...
################
### This, script2, is a copy of script1, but reading a different sheet from the same data file and making differently named outputs.
### By Garrett Janzen
################
...
##########################
data <- as.data.frame(read_xlsx("CEIRRFLU54.xlsx", sheet=3));dim(data) # This is the original data file, second sheet
...
pdf("Figures/Sheet2/s2_clade_value.pdf", width=12)
plot1
dev.off()
...
pdf("Figures/Sheet2/s2_decade_value.pdf", width=3.5, height=14)
plot_decade
dev.off()
...
pdf("Figures/Sheet2/s2_decade_value_cols.pdf", width=12, height=6)
plot_decadec
dev.off()
...
pdf("Figures/Sheet2/s2_decade_value_noclade.pdf", width=5, height=5)
plot_decadenc
dev.off()
...
#########################################################
### A figure made for a separate presentation, 1/10/2025
design_1B2x <- "
  ABC
"
data_sub_t_melt_decade_temp <- data_sub_t_melt_decade[which(data_sub_t_melt_decade$clade == "IA20.1B.2.2.1" |
                                                              data_sub_t_melt_decade$clade == "IA20.1B.2.2.2" |
                                                              data_sub_t_melt_decade$clade == "IA21.1B.2.1"),];dim(data_sub_t_melt_decade_temp)
plot_decadec_temp <- ggplot(data_sub_t_melt_decade_temp, aes(x=decade, y=value)) + 
  facet_manual(vars(clade), design = design_1B2x) +
  geom_boxplot(lwd=1.5) +
  labs(y = "Log2 (GMT/10)", x = "Decade") +
  geom_jitter(size=0.9, alpha=0.5) +
  geom_jitter(color="black", size=0.3, alpha=0.5) +
  scale_y_continuous(breaks=seq(0,10,2)) +
  theme(text = element_text(size = 20)); plot_decadec_temp
png("Figures/Sheet2/s2_decade_value_1B21.png", width=1200, height=400)
plot_decadec_temp
dev.off()
...
pdf("Figures/Sheet2/s2_gender_value.pdf", width=3.5, height=14)
plot_gender
dev.off()
...
pdf("Figures/Sheet2/s2_gender_value_cols.pdf", width=12, height=6)
plot_genderc
dev.off()
...
pdf("Figures/Sheet2/s2_gender_value_noclade.pdf", width=5, height=5)
plot_gendernc
dev.off()
...
pdf("Figures/Sheet2/s2_vaccine_value.pdf", width=3.5, height=14)
plot_vaccine
dev.off()
...
pdf("Figures/Sheet2/s2_vaccine_value_cols.pdf", width=12, height=6)
plot_vaccinec
dev.off()
...
pdf("Figures/Sheet2/s2_vaccine_value_noclade.pdf", width=5, height=5)
plot_vaccinenc
dev.off()
...
pdf("Figures/Sheet2/s2_qqplot_ci.pdf", width=5, height=5)
qqPlot(data_sub_t_melt_vaccine_nona$value, distribution="norm")
dev.off()
...
data$cohort <- "script2"
data_sub_t_melt_vaccine$cohort <- "script2"
data_sub_t_melt_decade$cohort <- "script2"
data_sub_t_melt_gender$cohort <- "script2"

write.csv(data, file="Output/script2_data.csv", row.names = F)
write.csv(data_sub_t_melt_vaccine, file="Output/script2_data_sub_t_melt_vaccine.csv", row.names = F)
write.csv(data_sub_t_melt_decade, file="Output/script2_data_sub_t_melt_decade.csv", row.names = F)
write.csv(data_sub_t_melt_gender, file="Output/script2_data_sub_t_melt_gender.csv", row.names = F)
