...
################
### This, script3, is a copy of script1, but reading a different sheet from the same data file and making differently named outputs.
### By Garrett Janzen
################
...
##########################
data <- as.data.frame(read_xlsx("FLU54_Master.xlsx", col_names=T, sheet=3));dim(data) # This is the second data file, third sheet, farm employees
# this data file is structured differently from the data file in scripts 1 and 2, so we have to do some extra steps to standardize things.
data <- data[,1:32];head(data);dim(data) #This drops a side-table of C1, C2, and C3 data, which we are not using.
data <- data[,-2];head(data);dim(data) #This drops a redundant count column
for(i in 1:18){
  print(data[1,i])
...
# Make and add the Decades to the data:
data$Date <- openxlsx::convertToDate(data$Date) # fixes dates
dates <- as.Date(data$Date)
data$Age <- as.numeric(data$Age)
dates_sub <- dates %m-% years(data$Age)
dates_sub <- as.numeric(substr(dates_sub,1,4))
decades <- dates_sub - dates_sub %% 10;table(decades)
data$Decade <- decades
...
pdf("Figures/Sheet3/s3_clade_value.pdf", width=12)
plot1
dev.off()
...
pdf("Figures/Sheet3/s3_decade_value.pdf", width=3.5, height=14)
plot_decade
dev.off()
...
pdf("Figures/Sheet3/s3_decade_value_cols.pdf", width=12, height=6)
plot_decadec
dev.off()
...
pdf("Figures/Sheet3/s3_decade_value_noclade.pdf", width=5, height=5)
plot_decadenc
dev.off()
...
pdf("Figures/Sheet3/s3_gender_value.pdf", width=3.5, height=14)
plot_gender
dev.off()
...
pdf("Figures/Sheet3/s3_gender_value_cols.pdf", width=12, height=6)
plot_genderc
dev.off()
...
pdf("Figures/Sheet3/s3_gender_value_noclade.pdf", width=5, height=5)
plot_gendernc
dev.off()
...
# data_sub_temp[data_sub_temp=="<10"]<-0
data_sub_temp <- data_sub_temp[-which(data_sub_temp$Vaccine == "Do not know"),] # This line is changed from Sheet 1
...
data1melt$value <- as.numeric(data1melt$value)
data2melt$value <- as.numeric(data2melt$value)
data3melt$value <- as.numeric(data3melt$value)
data4melt$value <- as.numeric(data4melt$value)
data5melt$value <- as.numeric(data5melt$value)
data6melt$value <- as.numeric(data6melt$value)
data7melt$value <- as.numeric(data7melt$value)
data8melt$value <- as.numeric(data8melt$value)
data9melt$value <- as.numeric(data9melt$value)
data10melt$value <- as.numeric(data10melt$value)
data11melt$value <- as.numeric(data11melt$value)
data12melt$value <- as.numeric(data12melt$value)
data13melt$value <- as.numeric(data13melt$value)
...
pdf("Figures/Sheet3/s3_vaccine_value.pdf", width=3.5, height=14)
plot_vaccine
dev.off()
...
pdf("Figures/Sheet3/s3_vaccine_value_cols.pdf", width=12, height=6)
plot_vaccinec
dev.off()
...
pdf("Figures/Sheet3/s3_vaccine_value_noclade.pdf", width=5, height=5)
plot_vaccinenc
dev.off()
...
pdf("Figures/Sheet3/s3_qqplot_ci.pdf", width=5, height=5)
qqPlot(data_sub_t_melt_vaccine_nona$value, distribution="norm")
dev.off()
...
data$cohort <- "script3"
data_sub_t_melt_vaccine$cohort <- "script3"
data_sub_t_melt_decade$cohort <- "script3"
data_sub_t_melt_gender$cohort <- "script3"

write.csv(data, file="Output/script3_data.csv", row.names = F)
write.csv(data_sub_t_melt_vaccine, file="Output/script3_data_sub_t_melt_vaccine.csv", row.names = F)
write.csv(data_sub_t_melt_decade, file="Output/script3_data_sub_t_melt_decade.csv", row.names = F)
write.csv(data_sub_t_melt_gender, file="Output/script3_data_sub_t_melt_gender.csv", row.names = F)
