...
################
### This, script4, is a copy of script1, but reading a different sheet from the same data file and making differently named outputs.
### By Garrett Janzen
################
...
##########################
data <- as.data.frame(read_xlsx("FLU54_Master.xlsx", col_names=T, sheet=4));dim(data) # This is the second data file, fourth sheet, general population
...
decades <- data$YOB - data$YOB %% 10;table(decades)
data$Decade <- decades

# For this dataset only, make a new column that indicates that all individuals were vaccinated in the 21-22 period.
data$Flu_Vax_2021_2022 <- "Y"
...
pdf("Figures/Sheet4/s4_clade_value.pdf", width=12)
plot1
dev.off()
...
pdf("Figures/Sheet4/s4_decade_value.pdf", width=3.5, height=14)
plot_decade
dev.off()
...
pdf("Figures/Sheet4/s4_decade_value_cols.pdf", width=12, height=6)
plot_decadec
dev.off()
...
pdf("Figures/Sheet4/s4_decade_value_noclade.pdf", width=5, height=5)
plot_decadenc
dev.off()
...
pdf("Figures/Sheet4/s4_gender_value.pdf", width=3.5, height=14)
plot_gender
dev.off()
...
pdf("Figures/Sheet4/s4_gender_value_cols.pdf", width=12, height=6)
plot_genderc
dev.off()
...
pdf("Figures/Sheet4/s4_gender_value_noclade.pdf", width=5, height=5)
plot_gendernc
dev.off()
...
colnames(data) # 19-ncol(data) are the genotypes
data_sub_temp <- data[,c("Flu_Vax_2021_2022",colnames(data)[20:ncol(data)-1])] # This line altered, due to the creation of the new vacc columnn earlier
...
data1melt[which(data1melt$vaccine=="Y"),]$value
log(data1melt[which(data1melt$vaccine=="Y"),]$value/10, 2)
...
pdf("Figures/Sheet4/s4_vaccine_value.pdf", width=3.5, height=14)
plot_vaccine
dev.off()
...
pdf("Figures/Sheet4/s4_vaccine_value_cols.pdf", width=12, height=6)
plot_vaccinec
dev.off()
...
pdf("Figures/Sheet4/s4_vaccine_value_noclade.pdf", width=5, height=5)
plot_vaccinenc
dev.off()
...
pdf("Figures/Sheet4/s4_qqplot_ci.pdf", width=5, height=5)
qqPlot(data_sub_t_melt_vaccine_nona$value, distribution="norm")
dev.off()
...
p.adjust(c(sum.aov_decade_noclade$Pr..F.[1],
           # sum.aov_vaccine_noclade$Pr..F.[1],
           sum.aov_gender_noclade$Pr..F.[1]), method="bonferroni")
...
data$cohort <- "script4"
data_sub_t_melt_vaccine$cohort <- "script4"
data_sub_t_melt_decade$cohort <- "script4"
data_sub_t_melt_gender$cohort <- "script4"

write.csv(data, file="Output/script4_data.csv", row.names = F)
write.csv(data_sub_t_melt_vaccine, file="Output/script4_data_sub_t_melt_vaccine.csv", row.names = F)
write.csv(data_sub_t_melt_decade, file="Output/script4_data_sub_t_melt_decade.csv", row.names = F)
write.csv(data_sub_t_melt_gender, file="Output/script4_data_sub_t_melt_gender.csv", row.names = F)
